...
void glprogram::setVar(const std::string&& name, const int& mCurrentProgramActive, std::vector<float>& vars, bool transpose) {
    ...
    {
        GLfloat* tmp = &vars[0];
        glUniform1fv(address, 4 * sizeof(GLfloat), tmp);
        break;
    }
    }
}
...
void glprogram::render()
{
    ...
    setVar("arr", frac->programId(), Constant, false);
...
void glprogram::change_constant()
{
    timex += 0.01;
    Constant[0] = sin((timex));
    Constant[1] = cos(timex);
}
