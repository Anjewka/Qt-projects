...
// Для удобства
void push_back(std::vector<float>& vec, const std::vector<glm::vec3>& vec2)
{
...
struct OpenglWindow
{
    ...
    void render()
    {
        float time = 0;
        // Вершины из задания
        std::vector<glm::vec3> A = {glm::vec3(0, 0, 0), glm::vec3(0, 1, 0), glm::vec3(0, 0, -1)};
        std::vector<glm::vec3> B = {glm::vec3(0, 3, 0), glm::vec3(0, 21.0 / 255.0, 21.0 / 255.0), glm::vec3(0, 0, -1)};
        std::vector<glm::vec3> C = {glm::vec3(3, 3, 0), glm::vec3(191.0 / 255.0, 0, 0), glm::vec3(0, 0, -1)};
        std::vector<glm::vec3> D = {glm::vec3(3, -3, 0), glm::vec3(1, 1, 0), glm::vec3(0, 0, -1)};
        std::vector<glm::vec3> E = {glm::vec3(0, -3, 0), glm::vec3(128.0 / 255.0, 0, 1), glm::vec3(0, 0, -1)};
        std::vector<glm::vec3> M = {glm::vec3(0, 0, 5), glm::vec3(1, 1, 1), glm::vec3(0, 0, -1)};

        // Нормали граней(векторное вросизведение сторон)
        glm::vec3 norm_side_2 = glm::normalize(glm::cross(D[0] - B[0], D[0] - M[0]));
        glm::vec3 norm_side_3 = glm::normalize(glm::cross(B[0] - C[0], B[0] - M[0]));
        glm::vec3 norm_side_4 = glm::normalize(glm::cross(C[0] - D[0], C[0] - M[0]));
        glm::vec3 norm_side_5 = glm::normalize(glm::cross(D[0] - E[0], D[0] - M[0]));
        // Вектор вершин, первые 3 эл. - вершины, следующие - цвет, последние - нормаль
