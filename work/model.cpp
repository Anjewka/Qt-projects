...
model::model()
{
    ...
    this->setMinimumHeight(650); this->setMinimumWidth(650);

    this->setWindowTitle("Модель");
...
void model::setSettings()
{
    ...
    for(int i = 0; i<q.size() ; ++i)
    {
        ...
        while(q[i]->getColor() == QColor(255, 247, 165))
        {
            q[i]->setColor(rand() % 256, rand() % 256, rand() % 256);
        }
    }
...
void model::AddNewQ()
{
    if(q.size() < 11)
    {
        ...
        while(newq->getColor() == QColor(255, 247, 165))
        {
            newq->setColor(rand() % 256, rand() % 256, rand() % 256);
        }
        q.push_back(newq);
    }
}
...
void model::DrawMenu()
{
    ...
    apply->setFont(QFont("Gabriola", 20));
...
void model::checkPositions()
{
    for(int i = 0; i<q.size() ; ++i)
    {
        ...
        if(q[i]->getX() >= this->width())
        {
            q[i]->setX(this->width() - 1);
        }
        if(q[i]->getY() <= 0)
        {
            q[i]->setY(1);
        }
        if(q[i]->getY() >= this->height())
        {
            q[i]->setY(this->height() - 1);
        }
    }
}
...
void model::DrawScene()
{
    ...
    p.setBrush(QColor(255, 247, 165));
    if(!con) {p.setBrush(QColor(255, 247, 165, 100));}
    ...
    for(int i = 0; i < q.size() ; ++i)
    {
        ...
        if(!con) {p.setBrush(QColor(q[i]->getColor().red(), q[i]->getColor().green(), q[i]->getColor().blue(), 100));}
...
void model::DrawEscapeMenu()
{
    ...
    btnPause->setGeometry(0.3 * this->width(), 0.3 * this->height(), 0.4 * this->width(), 0.05 * this->height());
    btnPause->setText("Пауза");
    setVisibleEnabled(true, btnPause);

    btnContinue->setFont(font);
    btnContinue->setGeometry(0.3 * this->width(), 0.35 * this->height(), 0.4 * this->width(), 0.05 * this->height());
    btnContinue->setText("Продолжить");
    setVisibleEnabled(true, btnContinue);

    btnAddnew->setFont(font);
    btnAddnew->setGeometry(0.3 * this->width(), 0.4 * this->height(), 0.4 * this->width(), 0.05 * this->height());
    btnAddnew->setText("Добавить заряд");
    setVisibleEnabled(true, btnAddnew);

    btnSwap->setFont(font);
    btnSwap->setGeometry(0.3 * this->width(), 0.45 * this->height(), 0.4 * this->width(), 0.05 * this->height());
    btnSwap->setText("Изменить силу");
    setVisibleEnabled(true, btnSwap);

    btnQuit->setFont(font);
    btnQuit->setGeometry(0.3 * this->width(), 0.55 * this->height(), 0.4 * this->width(), 0.05 * this->height());
    btnQuit->setText("Выйти");
    setVisibleEnabled(true, btnQuit);

    btnDelete->setFont(font);
    btnDelete->setGeometry(0.3 * this->width(), 0.5 * this->height(), 0.4 * this->width(), 0.05 * this->height());
    btnDelete->setText("Удалить заряд");
    ...
    lbl->setGeometry((this->width() / 2 - 60), 0.25 * this->height(), 120, 40);
